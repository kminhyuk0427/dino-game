...
# 게임 종료 후 10초 동안 순위표를 화면 중앙에 표시하는 함수
def display_rank_on_exit():
    rank = read_rank()
    rank_text = font.render("Rank", True, black)
    screen.blit(rank_text, (screen_width // 2 - 30, screen_height // 2 - 70))
    y = screen_height // 2 - 30
    for i, item in enumerate(rank, start=1):
        rank_entry = font.render(f"{i}. {item:.1f}kg", True, black)
        screen.blit(rank_entry, (screen_width // 2 - 30, y))
        y += 30
    pygame.display.flip()
    time.sleep(10)
...
while running:
    ...
    # 화면 그리기
    screen.blit(background_image, (0, 0)) # 배경
    screen.blit(mh_image, (mh_x, mh_y)) # 캐릭터
    for burger in burgers: # 햄버거
...
# 게임 종료 후 순위표를 10초 동안 화면에 표시
display_rank_on_exit()
